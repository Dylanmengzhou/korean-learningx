...
function TestPageContent() {
  ...
  const handlePlay = () => {
    ...
    // 处理单词，移除空格和标点符号
    const originalWord = words[index].korean;
    const processedWord = originalWord.trim().replace(/\s+/g, "");
    const cleanWord = processedWord.replace(/[^\w\s가-힣]/g, ""); // 移除非韩文字符和标点

    // 尝试方法顺序：原始单词 -> 移除空格 -> 移除标点和空格
    tryPlayAudio(originalWord, processedWord, cleanWord);
  ...
  // 尝试多种方式播放音频
  const tryPlayAudio = (original: string, processed: string, clean: string) => {
    ...
    audioRef.current.play().catch((err) => {
      console.error("播放原始单词音频出错，尝试备用方法1:", err);
      ...
      audioRef.current.play().catch((err2) => {
        console.error("备用方法1失败，尝试备用方法2:", err2);

        if (!audioRef.current) return;

        // 尝试3: 使用清理后的单词（移除标点和空格）
        audioRef.current.src = `https://dict.youdao.com/dictvoice?audio=${encodeURIComponent(
          clean
        )}&type=0&le=ko`;

        audioRef.current.play().catch((err3) => {
          console.error("所有播放方法都失败了:", err3);
          console.log("问题单词:", original);
          alert(`音频播放失败: "${original}"\n请尝试再次点击播放按钮`);
        });
      });
    });
  };
