 ...
 */
async function batchUpdateWordsStatus(updates: { id: number; status: number }[]) {
  try {
...
export default function StudyPage() {
  ...
  useEffect(() => {
    async function loadWords() {
      ...
      try {
        ...
        const initialStates: Record<number, "认识" | "不认识" | "模糊" | null> =
          {};
  ...
  // 2. 批量更新提交逻辑
  // ----------------------------
  async function handleBatchUpdate(updates: { id: number; status: number }[]) {
  ...
  const handleSelectState = (
    ...
    startTransition(() => {
      ...
      setUpdateBuffer((prev) => {
        const newBuffer = [...prev, { id: currentWord.id, status: statusValue }];
        ...
        <div className="flex flex-col items-center justify-center w-full gap-1 mt-16">
          ...
          <div ref={sliderRef} className="keen-slider w-full max-w-xl">
            {words.map((word, index) => {
              ...
              return (
                <div className="keen-slider__slide p-2" key={word.id}>
                  <Card
                    className={`
          ...
          {/* 显示/隐藏中文按钮 */}
          <div className="cursor-pointer" onClick={() => setVisiable(!visiable)}>
            {visiable ? <Eye /> : <EyeOff />}
          </div>
